...
const getMemberAddressByIdData = async () => {
  ...
  if (query.id) {
    // 发送请求
    const res = await AddressService.getMemberAddressDetailsAPI(query.id)
    // 把数据合并到表单中
...
const onSubmit = async () => {
  try {
    ...
    if (query.id) {
      //修改地址请求
      await AddressService.putMemberAddressByIdAPI(query.id, form.value)
    } else {
      //新建地址请求
      await AddressService.postMemberAddressAPI(form.value)
    }
